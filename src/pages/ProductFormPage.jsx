        ...
        <div className="border border-slate-200 rounded-xl overflow-hidden mb-3">
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-xs text-slate-500 uppercase tracking-wider">
              <tr>
                <th className="px-4 py-2.5 text-left font-semibold w-2/5">
                  Spec Name
                </th>
...
function ImagePicker({
  ...
  // Always start in upload mode.
  // Existing image will still be shown in preview.
  const [mode, setMode] =
    useState("upload");
...
function Toast({
  ...
  return (
    <div
      className={`fixed top-5 right-5 z-50 px-5 py-3 rounded-xl shadow-xl text-sm font-semibold
      flex items-center gap-2 transition-all animate-in ${
        styles[type] ||
        styles.info
      }`}
      style={{
        animation:
          "slideInRight .25s ease",
      }}
    >
      {type === "success"
        ? "✅"
...
export default function ProductFormPage() {
  ...
  useEffect(() => {
    ...
    productsAPI
      ...
      .then((r) => {
        ...
        // ====================================================
        // IMPORTANT:
        // Keep the existing image separately.
        // Do NOT put uploaded /uploads image into imageUrl.
        // ====================================================
        ...
        // Make sure a previous selected file
        // is cleared when loading product.
        setImageFile(null);
      })
      .catch((err) => {
        console.error(
  ...
  const handleSubmit =
    async (e) => {
      ...
      // Do not allow a NEW product
      // to use inactive category.
      //
      // For EDIT:
      // if the product already belongs to
      // an inactive category, we allow saving
      // the product without changing its category.
      ...
      if (
        ...
        setErrors((prev) => ({
          ...prev,
          category:
            "This category is inactive. Please select an active category.",
        }));
      ...
      try {
        ...
        //
        // Case 3:
        // Existing /uploads image and no new image
        // → DO NOT SEND imageUrl
        //
        // This is what prevents the image from being
        // deleted when only status is changed.
        // ====================================================
    ...
    <div className="max-w-4xl mx-auto">
      ...
      >
        ...
        <FormSection
          ...
          subtitle="Core product details shown on cards and search results"
        >
          ...
          <div>
            ...
            <Textarea
              ...
              rows={4}
              placeholder="Write a clear, helpful description of the product…"
              error={
                errors.description
              }
            />
        ...
        <FormSection
          title="Product Image"
          subtitle="Upload a file or paste an image URL"
        >
          <ImagePicker
            imageFile={
              imageFile
        ...
        <FormSection
          ...
          subtitle="Specifications and feature highlights shown on the detail page"
        >
          <ArrayField
            label="Short Specs (Summary chips)"
            subtitle="2–4 bullet points shown on product cards."
            items={
              shortSpecs
            }
            onChange={
        ...
        >
          <p className="text-xs text-slate-400 order-2 sm:order-1">
            {isEdit
              ? "Changes will update the live website immediately after saving."
              : "Product will go live after creation if marked Active."}
          </p>
